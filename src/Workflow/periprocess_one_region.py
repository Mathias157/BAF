...
def create_demand_response(
    ...
    for commodity in commodities:
        ...
        print(
            f"{log_time()} Starting to batch {regions} with current unserved energy costs: {regional_unserved_energy_costs}"
        )
        ...
        for region in regional_unserved_energy_costs.keys():
            print(
                f"{log_time()} setting unserved energy cost in {region} to {regional_unserved_energy_costs[region]}"
            )
...
def process_in_batches(
    ...
    for i in range(0, len(regions), batch_size):
        ...
        print(
            f"{log_time()} Processing batch {i // batch_size + 1}/{(len(regions) - 1) // batch_size + 1}"
        )
...
def model_demand_response(
    ...
    # Do kernel smoothing
    print(
        f"{log_time()} Kernel smoothing {parameter_x} and {parameter_y} for {commodity} in {region}"
    )
    bandwidth_x = 0.05
    bandwidth_y = 0.05
...
@click.pass_context
def main(ctx, sc_name: str, year: str, region: str):
    ...
    if sc_name == None:
        ...
        print(f"{log_time()} Reading SC from Config.ini..")
        Config = configparser.ConfigParser()
    ...
    # Loading MainResults
    print(
        f"{log_time()} Loading results for year %s from Balmorel/%s/model/MainResults_%s.gdx\n"
        % (year, SC_folder, SC)
    )
    res = MainResults(
...
def peri_process(ctx, scenario: str, year: str, region: str):
    try:
        main(scenario, year, region)
