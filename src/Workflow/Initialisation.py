...
print(f"Initialising scenario {SC}!\n")
...
print(f"Wrote Workflow/MetaResults/{SC}_meta.ini\n")

# 0.0 Load configuration file
SC_folder = Config.get("RunMetaData", "SC_Folder")
ResetReserveMargin = (
    Config.get("PostProcessing", "ResetReserveMargin").lower() == "true"
)
...
fAntTechno = pd.DataFrame({}, columns=["Iter", "Year", "Region", "Tech"]).pivot_table(
    index=["Iter", "Year", "Region", "Tech"]
)
...
if USE_MARKETVAL:
    # Market value
    fMV = pd.DataFrame(
        {}, columns=["Iter", "Year", "Region", "Tech", "Value (eur/MWh)"]
    )
...
if USE_FICTDEM:
    ...
    f = IncFile(
        ...
        body="\n".join(
            [
                f"DE('{year}',RRR,'FICTIVE_{year}') = {FictElFactorFunc}*SUM(DEUSER, DE('{year}',RRR,DEUSER));"
                for year in Y
            ]
        ),
        path=path,
    ...
    f = IncFile(
        prefix='SET DEUSER  "Electricity demand user groups. Set must include element RESE for holding demand not included in any other user group"\n/\n',
    ...
    f = IncFile(
        name="ANTBALM_FICTDE_VAR_T",
        body="\n".join(
            [
                f"DE_VAR_T(RRR, 'FICTIVE_{year}', SSS, TTT) = DE_VAR_T(RRR, 'RESE', SSS, TTT);"
                for year in Y
            ]
        ),
        path=path,
    ...
    f = IncFile(
        ...
        body="\n".join(
            [
                f"DH('{year}',AAA,'FICTIVE_{year}') = {FictHFactorFunc}*SUM(DHUSER, DH('{year}',AAA,DHUSER));"
                for year in Y
            ]
        ),
        path=path,
    ...
    # Fictive Electricity Demand 'Users' (actually just to have different profiles pr. year)
    f = IncFile(
        prefix='SET DHUSER  "Heat demand user groups. Set must include element RESH for holding demand not included in any other user group"\n/\n',
    ...
    # Fictive Electricity Demand Profiles
    f = IncFile(
        name="ANTBALM_FICTDH_VAR_T",
        body="\n".join(
            [
                f"DH_VAR_T(AAA, 'FICTIVE_{year}', SSS, TTT) = SUM(DHUSER, DH_VAR_T(AAA, DHUSER, SSS, TTT));"
                for year in Y
            ]
        ),
        path=path,
    ...
    f = IncFile(
        name="ANTBALM_FICTDH2",
        body="\n".join(
            [
                f"HYDROGEN_DH2('{year}',RRR) = {FictH2FactorFunc}*HYDROGEN_DH2('{year}',RRR);"
                for year in Y
            ]
        ),
        path=path,
...
if USE_CAPCRED:
    ...
    f = IncFile(
        name="ANTBALM_XCAPCRED",
        prefix="PARAMETER ANTBALM_XCAPCRED(YYY, IRRRE, IRRRI)\n;\n",
        body="\n".join(
            [
                "ANTBALM_XCAPCRED(YYY, IRRRE, IRRRI) = %d;" % capcred_first_iter,
    ...
    if USE_H2CAPCRED:
        ...
        f = IncFile(
            name="ANTBALM_XH2CAPCRED",
            prefix="PARAMETER ANTBALM_XH2CAPCRED(YYY, IRRRE, IRRRI)\n;\n",
            body="\n".join(
                [
                    "ANTBALM_XH2CAPCRED(YYY, IRRRE, IRRRI) = %d;" % capcred_first_iter,
