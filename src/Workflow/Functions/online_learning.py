...
@click.pass_context
@click.option(
    "--scenario-name", type=str, required=True, help="Name of the scenario to run"
)
@click.option(
    "--scenario-folder",
    type=str,
...
@click.option(
    ...
    default=".pdf",
    required=False,
    help='The extension of the plot, defaults to ".pdf"',
)
@click.option(
    "--pretrain-epochs",
    type=int,
...
def CLI(
    ...
    logger.info(
        f"scenario={scenario_name} pretrain_epochs={pretrain_epochs} update_epochs={update_epochs} "
    ...
    while epoch < update_epochs:
        ...
        for runtype in ["capacity", "dispatch", "rolling"]:
            ...
            if runtype == "capacity":
                ...
                sp.run(
                    f"mv {scenario_folder}/data/DE.inc {scenario_folder}/data/ANTBALM_FICTDE.inc",
                    shell=True,
                )
                sp.run(
                    f"mv {scenario_folder}/data/DH.inc {scenario_folder}/data/ANTBALM_FICTDH.inc",
                    shell=True,
                )
                sp.run(
                    f"mv {scenario_folder}/data/HYDROGEN_DH2.inc {scenario_folder}/data/ANTBALM_FICTDH2.inc",
                    shell=True,
                )
            else:
                sp.run(
