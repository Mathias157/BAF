...
@click.option('--overwrite', required=False, default=False, help="Load input files again? Defaults to False, use True if you changed data")
def main(scenario1: str, scenario2: str | None, overwrite: bool):
    ...
    full_resolution = pd.MultiIndex.from_product(([f'S{s:02.0f}' for s in range(1, 53)], [f'T{t:03.0f}' for t in range(1, 169)]))
    ...
    for incfile in ['WND_VAR_T', 'SOLE_VAR_T', 'DH_VAR_T', 'DE_VAR_T', 'WTRRRVAR_T', 'WTRRSVAR_S']:
        ...
        # Get dimensions
        vars = [col for col in df1.columns if col not in ['SSS', 'S', 'TTT', 'T', 'Value']]
        temporal_dimension = [col for col in df1.columns if col in ['SSS', 'S', 'TTT', 'T']]

        # Pivot
        df1=(
            df1
            .pivot_table(index=temporal_dimension, columns=vars, values='Value')
        ...
        for batch in range(0, len(df1.columns), 9):
            ...
            for col in columns:
                # Make duration curve
                if col != columns[-1]:
                    values1, bins1 = doLDC(df1[col].values, 100, plot=True, ax=ax[j][i], **{'color':'orange'})
                    values2, bins2 = doLDC(df2[col].values, 100, plot=True, ax=ax[j][i], **{'color':'k'})
                else:
                    values1, bins1 = doLDC(df1[col].values, 100, plot=True, ax=ax[j][i], **{'label':scenario1, 'color':'orange'})
                    values2, bins2 = doLDC(df2[col].values, 100, plot=True, ax=ax[j][i], **{'label':scenario2, 'color':'k'})
