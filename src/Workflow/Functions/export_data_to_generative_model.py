"""
Data Compilation for Online Learning Method

Script for compiling data for BALMOLRE into a uniform .csv
file that the neural network understands
...
def export_to_generative_model(
    scenario_folder: str,
    weather_year: int,
    filename: str,
    parameters: list = [
    ...
    **kwargs,
    ...
    model = Balmorel(
        "Balmorel", gams_system_directory=kwargs.pop("gams_system_directory", None)
    )
    ...
    if read_weather_year != weather_year:
        raise ValueError(
            f"Read weather year was {read_weather_year}, but process expect {weather_year}!"
        )
    ...
    for parameter in parameters:
        print(parameter)
        df = correct_format(
            symbol_to_df(model.input_data[scenario_folder], parameter),
            parameter,
            weather_year,
            IR,
            IA,
...
def correct_format(
    df: pd.DataFrame, parameter: str, weather_year: int, IR: list, IA: list
):
    ...
    df.loc[:, ["Parameter"]] = (
        parameter + "|" + df[unique_sets].astype(str).agg("|".join, axis=1)
    )
    ...
    if "SSS" in df.columns and "TTT" in df.columns:
        ...
        if len(ST_missing) > 0:
            ...
            missing_index = pd.MultiIndex.from_arrays(
                ([weather_year] * len(ST_missing), S_missing, T_missing),
                names=["WY", "SSS", "TTT"],
            )
        ...
        df = df.pivot_table(
            index=["WY", "SSS"], columns="Parameter", values="Value", fill_value=0
        )
        ...
        if len(S_missing) > 0:
            missing_index = pd.MultiIndex.from_arrays(
                ([weather_year] * len(S_missing), S_missing), names=["WY", "SSS"]
            )
            df = concat_missing(df, missing_index)
        df = stack_and_reset(df)
...
def main(scenario_folder: str):
    ...
    for weather_year in weather_years:
        ...
        export_to_generative_model(
            scenario_folder,
            weather_year,
            "Pre-Processing/Output/genmodel_data_WY%d.csv",
    ...
    for weather_year in weather_years:
        df = pd.concat(
            (
                df,
                pd.read_csv(
                    "Pre-Processing/Output/genmodel_data_WY%d.csv" % weather_year,
                    index_col=0,
                    header=[0, 1, 2],
                ).T,
            )
